...
contract Starter is Configurable {
    ...
    function purchase(uint amount) public virtual {
    ...
    function purchaseHT() public payable  virtual {
...
contract StarterLimit is Starter {  //IBO
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    ...
    uint public amtHigh;
    uint public maxUser;
    uint public curUserCount;
    uint public offerBegin;

    function setLimit(uint amtLow_, uint amtHigh_,uint maxUser_,uint offerBegin_) public governance {
        amtLow = amtLow_;
        amtHigh = amtHigh_;
        maxUser = maxUser_;
        offerBegin = offerBegin_;
    }

    function purchase(uint amount) public override {
        require(now>=offerBegin,'Not begin');
        require(purchasedCurrencyOf[msg.sender] == 0, 'already purchased');
        require(amount>=amtLow &&amount <= amtHigh, 'amount limit');
        require(curUserCount<maxUser, 'over count');
        curUserCount++;
        Starter.purchase(amount);
    }

    function purchaseHT() public payable override{
        require(now>=offerBegin,'Not begin');
        require(purchasedCurrencyOf[msg.sender] == 0, 'already purchased');
        require(msg.value>=amtLow && msg.value <= amtHigh, 'amount limit');
        require(curUserCount<maxUser, 'over count');
        curUserCount++;
        Starter.purchaseHT();
    }
